...
export function Cabinet() {
  ...
  const filtered = useMemo(() => {
    ...
    if (q) list = list.filter((m) => `${m.from} ${m.to} ${m.subject || ''} ${m.text || ''}`.toLowerCase().includes(q));
    return list;
  }, [view, temp.msgs, perm.msgs, query]);
  const activeMsg = useMemo(() => filtered.find((m) => m.id === sel) || null, [filtered, sel]);

  return (
    <div className="min-h-full">
      <TopBar onCompose={() => setCompose(true)} onMenu={() => setMenuOpen((v) => !v)} query={query} onQuery={(q) => { setQuery(q); }} />
      <div className="flex">
        <SideBar
          ...
          {view === 'boxes' && (
            <div className="bg-white rounded-3xl border border-[#dadce0] p-5 animate-in shadow-sm">
              ...
              <div className="mt-3 space-y-2">
                {perm.boxes.map((b) => (
                  <div key={b.id} className={`flex items-center gap-3 border rounded-2xl px-4 py-3 ${perm.activeId === b.id ? 'border-[#1a73e8] bg-[#e8f0fe]/40' : 'border-[#dadce0]'}`}>
                    <span className="material-symbols-outlined text-[#5f6368]">mail</span>
                    <span className="flex-1 break-all font-medium">{b.address}</span>
                    <button onClick={() => copyText(b.address, 'Адрес скопирован')} className="text-sm text-[#5f6368] hover:text-black">Копировать</button>
                    <button onClick={() => perm.setActiveId(b.id)} className="text-sm text-[#1a73e8] font-medium">Выбрать</button>
                    <button onClick={async () => { await perm.removeBox(b.id); toast('Ящик удалён'); }} className="text-sm text-red-600">Удалить</button>
                  </div>
                ))}
                {perm.boxes.length === 0 && (
                  <div className="text-sm text-[#5f6368] border border-dashed border-[#dadce0] rounded-2xl p-4 text-center">
                    Пока пусто. Придумайте имя слева — и у вас появится настоящая почта с отправкой.
                  </div>
                )}
              </div>
              {perm.boxes.length < MAX_PERMANENT_PER_USER ? (
                <div className="flex flex-col sm:flex-row gap-2 mt-4">
                  ...
                  <button
                    onClick={async () => {
                      setErr('');
                      try { await perm.createBox(local.trim(), pdom!); setLocal(''); toast('Ящик создан'); }
      ...
      {compose && activeBox && (
        <ComposeModal
          ...
          onSend={async (p) => { await perm.sendMail(activeBox.id, p); toast('Письмо отправлено'); }}
        />
      )}
      {compose && !activeBox && (
